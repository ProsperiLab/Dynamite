...
.packages <-  c("optparse", "remotes", "phytools", "data.tree", 
                ...
                "treeio", "ggtree", "parallel", "foreach", "geiger", "tibble", "treedater")  # May need to incorporate code for familyR (https://rdrr.io/github/emillykkejensen/familyR/src/R/get_children.R) i fno longer supported.
...
option_list = list(
  ...
  make_option(c("-m", "--metadata"), type="character", default=list.files(pattern="metadata.csv")[[1]], 
...
message("Converting substitution tree into timed tree using lsd2 and dates provided in metadata file. Please make sure the metadata file contains IDs in the first column and dates in the second column. All other columns can be filled as desired...")
checkForDates <- function(metadata) {
  ...
  if (isTRUE(max(sts) > Sys.Date())) {
    ...
    message(paste("The following sequences likely have incorret dates:",
...
message("Performinc ancestral state reconstruction...")
if (opt$asr == "Y"){
...
#message("Claculating some tree statistics....Please note that Skygrowth-based estimates of Re and/or R0 are not accurate for clusters with true R0>5 or highly-sampled transmission clusters.")
...
names(cluster_stats) <- names(clusters)
