...
p_Move_fct  <- function(t, current.in, host.count){
  ...
  if(current.in=="A" & host.count < 3){return(0)}
  if(current.in=="A" & host.count >= 3){return(0.00015)} # Wait a couple of weeks before initiation of clusters
...
n_contact_fct = function(t, current.in, host.count){
  ...
  if(host.count < 900 & current.in=="A") {return(abs(round(rnorm(1, 10, 1), 0)))}
  if(host.count < 100 & current.in=="B") {return(abs(round(rnorm(1, 4, 1), 0)))}
  if(host.count < 100 & current.in=="C") {return(abs(round(rnorm(1, 4, 1), 0)))}
  if(host.count < 100 & current.in=="D") {return(abs(round(rnorm(1, 4, 1), 0)))}
  
  if(host.count >= 900 & current.in=="A") {return(0)}
  if(host.count >= 100 & current.in=="B") {return(0)}
  if(host.count >= 100 & current.in=="C") {return(0)}
  if(host.count >= 100 & current.in=="D") {return(0)}
...
p_Trans_fct <- function(t, prestime, current.in, t_incub){
  if(t < t_incub){p=0}
  if(t >= t_incub & current.in=="A"){p=0.01}
  if(t >= t_incub & current.in=="B"){p=0.08}
  if(t >= t_incub & current.in=="C"){p=0.10*1*exp(-0.008*prestime)/((1-0.10)+0.10*exp(-0.008*prestime))} ##0.9 is the initial, 1 is the carrying capacity, -0.5 is the growth rate
  if(t >= t_incub & current.in=="D"){p=0.08*1*exp(0.015*prestime)/((1-0.08)+0.08*exp(0.015*prestime))}
  return(p)
}
...
SimulationSingle <- nosoiSim(type="single", # Number of hosts
                             ...
                             length.sim = 300, # Max number of time units (can be days, months, weeks, etc.)
...
ggsave("cov_network_cuminf_sim1_max10000.png", plot=last_plot())
...
ggsave("cov_network_sim1_max10000.png", plot=last_plot())
...
sampled.hosts <- sample(table.hosts$hosts.ID, 400, replace=F)
...
write.beast(sampled.trees, "sampled_trees_max10000.tree") #### NEED THIS OUTPUT####################################
